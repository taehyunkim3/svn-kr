...
- [x] 실제 SVN fixture에서 사라진 추가 예약을 정리해 저장소와 로컬 파일 내용이 변하지 않는지 확인한다.
- [x] 실제 NFD 신규 디렉터리를 추가·커밋하고 저장소에 동일 원문 경로와 파일 바이트가 생기는지 확인한다.
- [x] `swift test` 전체를 실행해 모든 테스트가 통과하는지 확인한다.
- [x] `git diff --check`, `git status --short`로 기존 변경 보존과 범위를 확인한다.
- [x] `./scripts/package-app.sh`로 앱을 패키징하고 코드 서명을 확인한 뒤 실행 중인 앱을 재시작한다.
