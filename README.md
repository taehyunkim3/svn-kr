...
정규화하면 같지만 한쪽이 로컬 작업 사본에만 남은 NFC/NFD 별칭이면, 커밋 전에 앱이 해당 별칭을 현재 작업 폴더 안에서 정리합니다. 파일이나 폴더의 이름을 바꾸지 않고 SVN의 기존 경로 별칭만 되돌린 뒤 다시 검사하므로, 같은 한글 경로가 별도 추가되는 일을 막을 수 있습니다. `변경 사항`의 `한글 경로 충돌` 행에서 `동일 한글 경로 정리`를 눌러 직접 실행할 수도 있습니다.

서버에 정규화하면 같은 서로 다른 경로가 실제로 함께 등록되어 어느 쪽이 맞는지 판단할 수 없는 경우에는 임의로 커밋하지 않습니다. 이 경우 `변경 사항`의 `한글 경로 충돌` 행에서 `경로 자동 복구…`를 누르고 비어 있는 새 폴더를 선택하세요. 앱은 같은 서버 위치를 새로 체크아웃한 뒤 실제 수정 파일, 신규 파일, 로컬에서 삭제된 파일만 새 작업 폴더에 재현하고 다시 충돌 여부를 검사합니다.
