...
정규화하면 같지만 SVN 내부에는 서로 다른 경로가 함께 존재하는 경우에는 임의로 커밋하지 않습니다. `변경 사항`의 `한글 경로 충돌` 행에서 `경로 자동 복구…`를 누르고 비어 있는 새 폴더를 선택하세요. 앱은 같은 서버 위치를 새로 체크아웃한 뒤 실제 수정 파일, 신규 파일, 로컬에서 삭제된 파일만 새 작업 폴더에 재현하고 다시 충돌 여부를 검사합니다.
