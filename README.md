...
서버에 정규화하면 같은 서로 다른 경로가 실제로 함께 등록되어 어느 쪽이 맞는지 판단할 수 없는 경우에는 임의로 커밋하거나 자동 복구하지 않습니다. 이 경우 `변경 사항`의 `한글 경로 충돌` 행에서 `경로 자동 복구…`가 비활성화되며, 서버의 중복 경로를 먼저 수동으로 정리해야 합니다. 서버 경로가 모호한 상태에서는 새 작업 폴더로의 복구도 진행할 수 없습니다.
